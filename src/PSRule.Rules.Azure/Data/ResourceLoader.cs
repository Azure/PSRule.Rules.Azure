...
public abstract class ResourceLoader
{
    ...
    protected static string GetContent(string name)
    {
        ...
        var fileStream = typeof(ResourceLoader).Assembly.GetManifestResourceStream(name) ?? throw new ArgumentException(string.Format(Thread.CurrentThread.CurrentCulture, PSRuleResources.ManifestResourceNotFound, name), nameof(name));
        using (fileStream)
