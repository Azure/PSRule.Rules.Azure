...
### Terraform plan expansion

<!-- module:version v1.48.0 -->

This configuration option determines if Terraform plan JSON files will automatically be expanded.
By default, Terraform plan JSON files will not be automatically expanded.
When enabled, PSRule will discover plan JSON files generated by `terraform show -json` and expand supported
Azure resources for analysis.

Terraform plan JSON files are expanded when PSRule cmdlets with the `-Format File` parameter are used.
See [Using Terraform](../using-terraform.md) for supported providers, resource conversion details, and current limitations.

Syntax:

```yaml title='ps-rule.yaml'
configuration:
  AZURE_TERRAFORM_PLAN_EXPANSION: bool
```

Default:

```yaml title='ps-rule.yaml'
# YAML: The default AZURE_TERRAFORM_PLAN_EXPANSION configuration option
configuration:
  AZURE_TERRAFORM_PLAN_EXPANSION: false
```

Example:

```yaml title='ps-rule.yaml'
# YAML: Set the AZURE_TERRAFORM_PLAN_EXPANSION configuration option to enable expansion
configuration:
